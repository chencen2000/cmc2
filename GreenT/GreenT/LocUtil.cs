...
namespace GreenT
{
    public static class LocUtil
    {
        ...
        {
            RegistryKey curLocInfo = Registry.CurrentUser.OpenSubKey("Greent" + @"\" + getAppName(element), false);  
        ...
        public static void SwitchLanguage(FrameworkElement element, string inFiveCharLang)
        {
            ...
            RegistryKey UserPrefs = Registry.CurrentUser.OpenSubKey("Greent" + @"\" + getAppName(element), true);  
                    if (UserPrefs == null)
            {
                // Value does not already exist so create it  
                RegistryKey newKey = Registry.CurrentUser.CreateSubKey("Greent");
